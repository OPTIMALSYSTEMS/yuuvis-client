...
const checkForBrummfuglTranslations = lang => {
  const brummfuglResourcePath = path.resolve(
    __dirname,
    'i18n-brummfugl',
    lang + '.json'
  );
  const flokfuglResourcePath = path.resolve(
    __dirname,
    '..',
    '..',
    '..',
  ...
  const brummfugl = JSON.parse(
    fs.readFileSync(brummfuglResourcePath, { encoding: 'utf8' })
  );
  const flokfugl = JSON.parse(
    fs.readFileSync(flokfuglResourcePath, { encoding: 'utf8' })
  );
